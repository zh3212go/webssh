...
""",
    ...
    def test_custom_namespace(self):
        loader = DictLoader({"login.html": "{{ inc(5) }}"}, namespace={"inc": lambda x: x + 1})
        self.assertEqual(loader.load("login.html").generate(), b"6")
...
class StackTraceTest(unittest.TestCase):
    def test_error_line_number_expression(self):
        loader = DictLoader({"login.html": """one
two{{1/0}}
three
        """})
        try:
            loader.load("login.html").generate()
            self.fail("did not get expected exception")
        except ZeroDivisionError:
            self.assertTrue("# login.html:2" in traceback.format_exc())
